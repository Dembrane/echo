...
def _room_bundle(settings: dict) -> dict:
    return service.build_bundle(
        state=service.fresh_state(),
        settings=settings,
        report={"id": "presentation", "date_created": None},
        project={"id": "project", "language": "en"},
        participant_base_url="https://participants.example",
    )["files"]["session.json"]


def test_screen_theme_defaults_to_light_and_normalizes_anything_else() -> None:
    assert service.default_settings(title="Room")["theme"] == "light"
    assert service.normalize_settings({}, fallback_title="Room")["theme"] == "light"
    assert service.normalize_settings({"theme": "dark"}, fallback_title="Room")["theme"] == "dark"
    for junk in ("DARK", "midnight", "", None, True, 1, {"mode": "dark"}):
        settings = service.normalize_settings({"theme": junk}, fallback_title="Room")
        assert settings["theme"] == "light"


def test_screen_theme_patch_reaches_the_room_bundle() -> None:
    light = service.default_settings(title="Room")
    assert _room_bundle(light)["theme"] == "light"

    dark = service.merge_settings(light, {"theme": "dark"}, fallback_title="Room")
    assert dark["theme"] == "dark"
    assert _room_bundle(dark)["theme"] == "dark"

    # A patch that leaves the theme alone keeps the host's choice.
    kept = service.merge_settings(dark, {"show_qr": True}, fallback_title="Room")
    assert kept["theme"] == "dark"
    # And an explicit return to light is honoured.
    back = service.merge_settings(dark, {"theme": "light"}, fallback_title="Room")
    assert _room_bundle(back)["theme"] == "light"


def test_draft_theme_shows_in_the_draft_room_only_until_publish(monkeypatch) -> None:
    from unittest.mock import AsyncMock
    ...
    asyncio.run(present.save_draft(report, patch={"theme": "dark"}, expected_revision=0))

    state = asyncio.run(present.draft_state(report))
    assert state["settings"]["theme"] == "dark"
    assert state["published"]["theme"] == "light"
    # The draft preview is built from the draft's own settings; the room keeps
    # the published theme until the host publishes.
    assert _room_bundle(state["settings"])["theme"] == "dark"
    assert _room_bundle(state["published"])["theme"] == "light"

    asyncio.run(present.publish_draft(report, expected_revision=1))

    published = service.normalize_settings(config["popcorn_settings"], fallback_title="Published")
    assert published["theme"] == "dark"
    assert _room_bundle(published)["theme"] == "dark"
