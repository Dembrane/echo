...
def _answer_data_flags_high_risk(answers: object) -> bool:
    """Derive the high-risk flag from a list of questionnaire answer entries.

    Mirrors the q2 branch of onboarding `_flag_review` (ISSUE-012): a `q2` of
    "yes"/"true" (string or bool) marks a high-risk context. Tolerant of any
    shape; returns False on anything unexpected so it can never falsely nudge.
    """
...
async def is_high_risk_context(app_user_id: str) -> bool:
    ...
    Reads Wave D's `app_user.onboarding_answer_json` (ISSUE-012), whose shape is
    `{"version": ..., "data": [...answer entries...]}`. High-risk is derived from
    the `q2` answer the same way onboarding `_flag_review` does. Defensive: any
    missing record, malformed JSON, or lookup error reads as not-high-risk, so
    the additive Inbox nudge stays dormant rather than firing wrongly. It only
    appears when this returns True AND the user has no active license, and it
    warns, never blocks.
    """
    try:
        rows = await async_directus.get_items(
            ...
            {
                "query": {
                    "filter": {"id": {"_eq": app_user_id}},
                    "fields": ["id", "onboarding_answer_json"],
                    "limit": 1,
                }
            },
        )
    ...
    if not isinstance(rows, list) or not rows:
        return False
    answer_json = rows[0].get("onboarding_answer_json")
    if not isinstance(answer_json, dict):
        return False
    return _answer_data_flags_high_risk(answer_json.get("data"))
