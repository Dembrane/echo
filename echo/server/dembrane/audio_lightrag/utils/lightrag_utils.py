...
async def run_segment_id_to_conversation_id(segment_id: int) -> tuple[str, str]:
    ...
    query = {
        "query": {"filter": {"id": {"_in": conversation_chunk_ids}}, "fields": ["conversation_id"]}
    }
...
async def get_conversation_chunk_ids_from_segment_ids(
    db: PostgreSQLDB, segment_ids: list[int]
) -> dict[int, str]:
    ...
    sql = SQL_TEMPLATES["GET_CONVERSATION_CHUNK_IDS_FROM_SEGMENT_IDS"].format(
...
async def get_segment_from_conversation_chunk_ids(
    db: PostgreSQLDB, conversation_chunk_ids: list[str]
) -> list[int]:
    ...
    if conversation_chunk_ids == []:
        return []
    conversation_chunk_ids = ",".join(
        ["UUID('" + conversation_id + "')" for conversation_id in conversation_chunk_ids]
    )  # type: ignore
    sql = SQL_TEMPLATES["GET_SEGMENT_IDS_FROM_CONVERSATION_CHUNK_IDS"].format(
        conversation_ids=conversation_chunk_ids
    )
    result = await db.query(sql, multirows=True)
    if result is None:
        return []
    return [
        int(x["conversation_segment_id"])
        for x in result
        if x["conversation_segment_id"] is not None
    ]
...
async def get_segment_from_project_ids(db: PostgreSQLDB, project_ids: list[str]) -> list[int]:
    ...
    flat_conversation_ids: list[str] = [
        item for sublist in conversation_ids for item in sublist if item is not None
    ]
...
async def with_distributed_lock(
    ...
    while not lock_acquired and retries < max_retries:
        ...
        if lock_acquired:
            ...
            logger.info(
                f"Waiting for distributed lock (attempt {retries + 1}/{max_retries}): {lock_key}"
            )
            retries += 1
...
async def upsert_transcript(
    ...
    if id is None:
        ...
        id = (
            str(document_id)
            + "_"
            + str(int(hashlib.sha256(s.encode("utf-8")).hexdigest(), 16) % 10**8)
        )
...
async def fetch_query_transcript(
    db: PostgreSQLDB, query: str, ids: list[str] | str | None = None, limit: int = 10
) -> list[str] | None:
    if ids is None:
        ids = "NULL"
        filter = "NULL"
...
def fetch_segment_ratios(response_text: str) -> dict[int, float]:
    ...
    segment2count = {
        int(segment_id.split("_")[-1]): count for segment_id, count in segment_count.items()
    }
    total_count = sum(segment2count.values())
    return {k: v / total_count for k, v in segment2count.items()}


async def get_ratio_abs(
    rag_prompt: str, return_type: Literal["segment", "chunk", "conversation"]
) -> Dict[str, float]:
...
async def get_conversation_details_for_rag_query(
    ...
    if ratio_abs:
        ...
        conv_meta = {
            c["id"]: c
            for c in directus.get_items(
                "conversation",
                {
                    "query": {
                        "filter": {"id": {"_in": list(ratio_abs.keys())}},
