...
reasoning given, every part it names and every qualification.
...
2. `proposed_statement`: for a sub-group of two or more, one to three plain
   ...
   numbers, contrasts or claims that any member did not make, and must not drop
   any that a member did make. For a sub-group of one, repeat that member's
   statement.
...
- Parts and wholes: a member that names several parts of something and a
  member that names one of them, or sets one of them aside, are different
  arguments. "Good maintenance includes fixing what breaks and planning
  renewals" and "Good maintenance means planning renewals, not merely fixing
  what breaks" are different arguments: the first counts fixing what breaks as
  part of maintenance, the second sets it apart. A part is never the whole.
- Contrast and inclusion words: read every "including", "as well as", "both",
  "only", "just", "merely", "not just", "not merely", "rather than", "instead
  of", "distinct from", "unlike" and "beyond" in each member and in your
  proposed statement. When one member includes something that another member
  contrasts itself with, excludes or calls insufficient, they take different
  positions on it, even when they share their main point.

Before you mark a check `equivalent`, compare the proposed statement with the
member once more for these words: a proposed statement that adds a contrast
("distinct from", "not merely", "rather than") that the member did not make, or
drops a part the member named, is `not_equivalent` for that member.
...
Return only the JSON in the structured format requested by the caller.
