...
- On the stakeholders slide the map and its `.flow-key` own exactly the visible stage: the stage runs flush (`.stage.stage-fold`, padding zero) and a `.stake-fold` column inside it is `calc(100dvh - var(--deck-chrome))`, with a `100vh` declaration before it as the fallback. `measureDeckChrome()` in `app.js` sums the on-screen heights of `.topbar`, `.session-notice` and `.colophon` into `--deck-chrome`, so the Present shell (where all three are hidden) measures zero and the fold is the whole iframe. The map is `flex: 1 1 auto` with `height: auto`, which drops the dense inline height in this position, so `buildStakeMap` reads the taller container and re-solves at the new `W x H` (its cache is keyed by size, and the existing debounced resize re-render still applies). "Who you could involve next" is a sibling below the fold with its own generous padding, reached by scrolling. An open group's deck is unchanged: there the map stays in `.deck-tail` at its own `clamp` height, dense inline height included.
- Inside the Present shell the deck shares the room's side inset: `.present-shell` redefines `--pad-x` as `clamp(12px, 2vw, 36px)`, the same value the React room screen uses for its header, footer and map tab (`--audience-pad-x` in `AudienceScreen.module.css`). Everything the deck insets by `--pad-x` follows: the stage, the deck slides, the deck and popcorn tails, the keys strip, the stakeholders fold and its "who you could involve next", and the opening dialog. The deck iframe is flush to the left edge of the room, so the two edges line up exactly whenever the iframe is as wide as the window; with the QR card open the iframe is narrower, its own `2vw` is a few pixels smaller, and the match is approximate in the middle of the clamp. An exact match there would need the shell to pass its measured inset into the embed. Standalone and sample decks keep `clamp(24px, 4vw, 72px)`.
