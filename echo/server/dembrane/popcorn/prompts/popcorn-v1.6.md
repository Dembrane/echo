...
Weight is relative within the response, and it is a judgment, not a default.

- `3`: the phrase of the conversation. Use at most once, and only when one idea
  clearly organizes a substantial conversation. When one does, give it 3; do
  not leave every phrase at 2 because 2 is safe. A response with no 3 says
  that no single idea organized the conversation, which is sometimes true.
