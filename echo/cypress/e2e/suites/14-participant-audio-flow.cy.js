...
describe("Participant Audio Recording Flow", () => {
	...
	const dashboardBaseUrl =
		Cypress.env("dashboardUrl") || "https://dashboard.echo-next.dembrane.com";
				...
				return cy
					...
					.then((projects) => {
						...
						if (!lastProject || !lastProject.id) {
							throw new Error(
								"projectId not found. Ensure the create step completed successfully.",
							);
						}
	...
	const getPortalUrl = () =>
		`${portalBaseUrl}/${portalLocale}/${projectId}/start`;
	...
	it("records audio in the participant portal", () => {
		...
		cy.get('[data-testid="portal-onboarding-mic-skip-button"]').should(
			"be.visible",
		);
		...
		cy.get("body", { timeout: 30000 }).then(($body) => {
			if (
				$body.find('[data-testid="portal-audio-stop-button"]:visible').length >
				0
			) {
	...
	it("verifies transcription and cleans up", () => {
		...
		resolveProjectId().then(() => {
			cy.visit(
				`${dashboardBaseUrl}/${portalLocale}/projects/${projectId}/overview`,
			);
		});
		...
		cy.xpath(
			'//div[contains(@class, "mantine-Paper-root")]//div[contains(@style, "flex")]//div/p[contains(@class, "mantine-Text-root")]',
		)
