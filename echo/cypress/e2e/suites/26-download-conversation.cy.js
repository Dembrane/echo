...
describe("Download Conversation Flow", () => {
	...
	it("should upload audio file, download conversation audio, delete project, and logout", () => {
		...
		cy.get('[data-testid="conversation-download-audio-button"]')
			...
			.then((href) => {
				expect(href, "conversation audio href").to.be.a("string").and.not.be
					.empty;
		...
		cy.then(() => {
			...
			cy.request(signedUrlEndpoint).then((response) => {
				...
				expect(response.body, "signed merged mp3 url").to.be.a("string").and.not
					.be.empty;
				expect(response.body, "signed merged mp3 url format").to.match(
					/^https?:\/\//,
				);
