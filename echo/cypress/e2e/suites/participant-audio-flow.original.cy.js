	...
	it("should record audio using fake device and verify transcription", () => {
		...
		cy.then(() => {
			...
			cy.readFile("assets/videoplayback.mp3", "base64").then((mp3Base64) => {
				cy.origin(
					portalBaseUrl,
					{ args: { mp3Base64, portalUrl, projectId } },
					({ portalUrl, projectId, mp3Base64 }) => {
						const installMediaStubs = (win) => {
							...
							const patchAudioContext = (AudioContextCtor) => {
								...
								const originalCreateAnalyser =
									AudioContextCtor.prototype.createAnalyser;
							...
							const applyMediaStubs = () => {
								...
								if (win.navigator.permissions) {
									safeDefine(win.navigator.permissions, "query", () =>
										Promise.resolve({
											onchange: null,
											state: "granted",
										}),
								...
								safeDefine(win.navigator.mediaDevices, "enumerateDevices", () =>
									Promise.resolve(fallbackDevices),
								);

								safeDefine(win.navigator.mediaDevices, "getUserMedia", () =>
						...
						cy.get('input[placeholder="Group 1, John Doe, etc."]').type(
							"Audio Test Session",
						);
						...
						cy.contains("button", "Stop", { timeout: 20000 }).should(
							"be.visible",
						);
