...
import {
	...
	navigateToHome,
	updateProjectName,
} from "../../support/functions/project";
...
describe("Project Create, Edit, and Delete Flow", () => {
	...
	const portalBaseUrl = (
		Cypress.env("portalUrl") || "https://portal.echo-next.dembrane.com"
	).replace(/\/$/, "");
	...
	const registerExceptionHandling = () => {
		cy.on("uncaught:exception", (err) => {
			if (
				err.message.includes("Syntax error, unrecognized expression") ||
	...
	const buildDashboardProjectUrl = (id, page) => {
		...
		const baseHasLocale = basePathSegments.some((segment) =>
			localeSegmentPattern.test(segment),
		);
	...
	const clickVisibleCopyLinkButton = () => {
		cy.get('[data-testid="project-copy-link-button"]').then(($buttons) => {
			const $visibleButtons = $buttons.filter((index, button) =>
	...
	const resolveProjectId = () => {
		return cy
			.then(() => {
				...
				return cy
					.readFile("fixtures/createdProjects.json", { log: false })
					.then((projects) => {
						const lastProject = Array.isArray(projects)
							? projects[projects.length - 1]
							: null;
						if (!lastProject || !lastProject.id) {
							throw new Error(
								"projectId not found. Ensure the setup test completed successfully.",
							);
						}
	...
	it("Part 1: Admin Setup - Create Project and Configure Portal", () => {
		...
		cy.location("pathname").then((pathname) => {
			...
			if (projectIndex !== -1 && pathSegments[projectIndex + 1]) {
				...
				cy.get("main").within(() => {
					cy.get(`a[href*="${createdProjectId}"]`)
						.first()
				...
				cy.get('[data-testid="project-breadcrumb-name"]').should(
					"contain.text",
					newProjectName,
				);
				...
				cy.window()
					.then((win) => {
						if (
							...
							typeof win.navigator.clipboard.readText === "function"
						) {
	...
	it("Part 2: Participant Flow - Text Typing (Public URL)", () => {
		...
		const textContent =
			"This is a test response typed into the participant portal.";
		...
		if (thankYouContent) {
			cy.get('[data-testid="portal-finish-custom-message"]')
				.contains(thankYouContent)
				.should("be.visible");
		} else {
