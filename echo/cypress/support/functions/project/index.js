export const createProject = () => {
	...
	cy.get('[data-testid="project-home-create-button"]')
		.should("be.visible")
		.click();
	...
	cy.window().then((win) => {
		...
		if (isMobile) {
			...
			cy.url().then((currentUrl) => {
				const locale = currentUrl.includes("/en-US/")
					? "en-US"
					: currentUrl.includes("/nl-NL/")
						? "nl-NL"
						: "en-US";
			...
			cy.get('[data-testid="project-breadcrumb-home"]')
				.filter(":visible")
				.first()
				.click();
		}
...
export const deleteProject = (projectId) => {
	...
	cy.get('[data-testid="project-actions-delete-button"]')
		.scrollIntoView()
		.should("be.visible")
		.click();
	cy.wait(5000);
...
export const exportProjectTranscripts = () => {
	...
	cy.get('[data-testid="project-export-transcripts-button"]')
		.scrollIntoView()
		.click({ force: true });
};
