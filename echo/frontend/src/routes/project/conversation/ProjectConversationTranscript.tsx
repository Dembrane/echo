...
function alignEnhancedToRawTimes(
	enhancedText: string,
	rawWords: Word[],
): {
	...
	const rawTokens = rawWords.map((w, i) => ({
		norm:
			(w.confidence ?? 1.0) >= confidenceThreshold
				? normalizeToken(String(w.text ?? ""))
				: "",
	...
	const matchedTimes: Array<{ start: number; end: number } | null> = new Array(
		enhTokens.length,
	).fill(null);
			...
			for (let i = 0; i < part.value.length; i++) {
				if (rp < rawTokens.length && ep < enhTokens.length) {
					if (enhNorms[ep]) {
						...
						matchedTimes[ep] = {
							start: rawTokens[rp].start,
							end: rawTokens[rp].end,
						};
					}
				}
				rp++;
...
function paragraphsFromAlignment(
	enhancedText: string,
	rawWords: Word[],
): Segment[] {
	...
	const { matchedTimes, totalDurationSec } = alignEnhancedToRawTimes(
		enhancedText,
		rawWords,
	);
	...
	const distributeTimeProportionally = (
		...
		if (totalChars > 0) {
			...
			// Fallback for empty/no-text segments
	...
	if (firstKnown === -1 || lastKnown === -1) {
		...
		if (lastKnown < n - 1) {
			...
			distributeTimeProportionally(
				segs.slice(lastKnown + 1),
				startTime,
				duration,
			);
		}
	...
	for (const s of segs) {
		...
		s.endSec = Math.max(
			s.startSec + 0.2,
			Math.min(s.endSec, totalDurationSec || s.startSec + 0.2),
		);
		last = s.endSec;
	}
...
export const ProjectConversationTranscript = () => {
	...
	const useMergedView = hasMergedAudio && !legacyView;
	const showMergedAudioPlayer = useMergedView && showAudioPlayer;
	...
	// Modals
	const [
		downloadTranscriptModalOpened,
		{ open: openDownloadTranscriptModal, close: closeDownloadTranscriptModal },
	] = useDisclosure(false);
	...
	const mergedDiarization = useMemo(() => {
		if (!useMergedView || !allChunks.length) return null;
		const rawTexts = allChunks
			.map((c) => c.diarization?.data?.raw?.text)
			.filter(Boolean);
		const notes = allChunks
			.map((c) => c.diarization?.data?.note)
			.filter(Boolean);
	...
	return (
		<Stack>
			...
			{showMergedAudioPlayer && (
				<Paper
					p="md"
					className="sticky top-2 z-40 bg-white shadow-lg"
					withBorder
				>
					<Stack gap="xs">
						...
						{mergedAudioError && (
							<Alert color="red" variant="light">
								<Trans>
									We couldn't load the audio. Please try again later.
								</Trans>
							</Alert>
						)}
						{mergedPlayerReady && (
			...
			<Stack>
				<Group justify="space-between" align="center">
					...
					<Group gap="sm">
						...
						{!!conversationQuery.data?.is_finished && (
							<Switch
								checked={!legacyView}
						...
						<Tooltip label={t`Download transcript`}>
							<ActionIcon
								onClick={openDownloadTranscriptModal}
								size="md"
								variant="subtle"
								color="gray"
							>
				...
				{/* Download Modal */}
				<Modal
					opened={downloadTranscriptModalOpened}
					onClose={closeDownloadTranscriptModal}
				...
				>
					<Stack>
						...
						<Button
							onClick={() => {
								handleDownloadTranscript(filenameDownload);
								closeDownloadTranscriptModal();
							}}
							...
							return (
								<div key={chunk.id} ref={isLast ? loadMoreRef : undefined}>
									<ConversationChunkAudioTranscript
										...
										showAudioPlayer={legacyView && showAudioPlayer}
										isActive={
											useMergedView ? activeChunkId === chunk.id : false
										}
										onSeek={useMergedView ? seekAbs : undefined}
										currentTime={useMergedView ? currentTime : undefined}
										chunkOffsetStart={useMergedView && o ? o.start : 0}
										legacyView={legacyView}
