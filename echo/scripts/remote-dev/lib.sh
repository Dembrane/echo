...
RD_CELLD_ENABLE_HINT='re-run ./scripts/remote-dev.sh init and answer y to "Run celld?" (or add docker-compose-s3.yml and docker-compose-celld.yml to RD_COMPOSE_FILES in local.env), then ./scripts/remote-dev.sh up --skip-setup'
celld_enabled() {
    case " $RD_COMPOSE_FILES " in
