...
Host decision, September 19th 2026, replacing the letter-by-letter interpolation (it read as a terminal) and the lens tried after it. A popcorn pops, after the host's storyboard (pop..., tension builds, wiggle!, Explode!, Land, FLLLLIP, Land). A kernel, a small and slightly odd circle of the phrase's colour with no outline, appears where the phrase is about to land and draws the eye; it sits there while the tension builds, wiggles for a split second, and the phrase explodes out of it off the screen towards the room on the Z axis, then lands at its tilt. Translations stack: for each language the host asked for it pops again, tumbles top over bottom in the air (edge-on at the top, a thin line; about the upright axis a wide phrase swung through too much depth), and lands leaning the other way showing that language, the room's own words first and the others in random order until all have had their read. A host may ask for up to four languages in all. Translations arrive batch by batch over the existing event stream; a language that lands late gets its own pop, or the next fair slot if the phrase has left the stage. A phrase shown in translation ends with Phosphor's translate icon, and the language code when several languages are on the go. Do not show Original or Translation labels underneath Popcorn. Avoid random letter scrambling, flashing, prolonged blur or a soft whole-phrase wobble. Preserve complete legible wording in each settled state. Reserve enough space for both language variants using the existing overlap guards, so a longer translation does not collide with another popcorn or force an abrupt layout jump. Quotation styling must remain accurate to the visible text; translation provenance stays available in the evidence view.

Proposed prototype timings, to tune on a projector rather than treat as final constants:

- Entrance: about 1.3 s from kernel to landing, the words readable from about 0.9 s; the first read interval starts then. Each further language: about 0.8 s for the jump and the turn.
