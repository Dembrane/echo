...
> ○ Private workspace
>
> *(shown when "Open to the team" is selected:)*
> Who on the team inherits access?
> ☑ Team admins *(always inherit — can't uncheck)*
> ☐ Team members *(optional)*

Private workspace copy: *"Only you will have access. Team admins won't be auto-added, even when joining the team later."*

Dry-run line live-updates with the choice: *"3 team admins + 7 team members will inherit access."* or *"3 team admins will inherit access."* or *"Only you. Team admins won't inherit."*

**Show step 2 even for solo teams** (team of 1). Dry-run honestly reads "0 team admins will inherit" — don't hide the affordance.
...
> ◉ Workspace — *12 people in [workspace name] will get access, including 3 inherited team admins.*
...
4. **Error/empty states**: what does "0 team admins will be added" look like (solo team creating first workspace)?

---

## Ask 1 — Team admin page

1. **URL naming**: you mocked `/org/:orgId/members`. Our internal code says `org`, our user copy says "team". Keep URL as `/org/...` (shorter, matches code) or `/team/...` (matches copy)? Low stakes — pick one.
2. **"Invite to team" CTA**: I'm assuming this always sets `include_org_membership=true` so the invitee joins the team and auto-inherits every **open** workspace. Confirm? (Workspace-scoped invites still live inside each workspace's settings.)
...
5. **Project management on the team page — NEW ASK.** Delete-workspace is blocked if the workspace has any non-deleted project (decision locked 2026-04-20). So from the team admin page, an owner/admin needs to be able to see every project across every workspace in the team and soft-delete any of them — otherwise winding down a partner engagement means walking into 20 workspaces one by one. Pick the right surface:
    - **(i)** Third view on the team page: **List · Matrix · Projects**. Projects view = flat list of every project, filterable by workspace, row action = delete. Feels consistent with the existing view switcher.
    ...
    My lean: **(i)** — reuses the view switcher pattern you already drew and keeps team management on one URL. Confirm?

---

## Ask 2 — Tier management

5. **"Request upgrade" CTA target**: when a team admin clicks this, what happens?
